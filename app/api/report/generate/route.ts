...
export async function POST(req: NextRequest) {
  try {
    ...
    // Fallback: if no pinned runs, get recent 50 runs (covers deep-dive mode with 40 steps + AI summaries)
...
The above are comprehensive analysis summaries generated by the AI during exploration. These contain valuable context, patterns, and insights discovered during the analysis process.
`
    : ""
}
...
${
  explorationSummary
    ? `
Quick Insights (one-sentence per query):
${explorationSummary}
`
    : ""
}
...
- Leverage the AI analysis summaries above for comprehensive context and discovered patterns
...
- Synthesize insights from both the detailed SQL results AND the AI analysis summaries
